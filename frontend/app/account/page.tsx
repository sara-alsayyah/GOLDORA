            ...
            <div className="luxury-card rounded-[34px] p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <input
                  ...
                  onChange={(event) => setProfile((current) => (current ? { ...current, first_name: event.target.value } : current))}
                  placeholder="First name"
                  className="rounded-[18px] border border-[rgba(143,108,29,0.16)] bg-white/72 px-4 py-3 outline-none"
                />
                <input
                  value={profile?.last_name ?? ""}
                  onChange={(event) => setProfile((current) => (current ? { ...current, last_name: event.target.value } : current))}
              ...
              <button onClick={saveProfile} className="gold-button mt-5 rounded-full px-5 py-3 text-sm uppercase tracking-[0.18em]">
                Save changes
              </button>
            </div>
    ...
    {!Array.isArray(orders) || orders.length === 0 ? (
      <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-[rgba(143,108,29,0.2)] bg-white/50 p-8 text-center">
        ...
        <button
          ...
          className="gold-button mt-5 rounded-full px-6 py-2 text-xs uppercase tracking-[0.18em]"
        >
          Start shopping
        </button>
      </div>
      ...
      orders.map((order) => {
        ...
        return (
          <div
            key={order.id}
            className="rounded-[24px] border border-[rgba(143,108,29,0.14)] bg-white/66 p-5"
          >
            <div className="flex items-center justify-between gap-4">
              <div>
              ...
              <div className="text-right">
                <p className="text-sm uppercase tracking-[0.18em] text-[var(--gold-deep)]">
                  {order.status}
                </p>
